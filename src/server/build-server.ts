...
/**
 * Builds a fresh MCP server instance with every QuickBooks tool registered.
 *
 * Returns a NEW instance on each call (unlike the QuickbooksMCPServer singleton)
 * so that transports which require one server per connection — e.g. the stateless
 * Streamable HTTP transport, which creates a server per request — get an isolated
 * instance. RegisterTool still honours the QUICKBOOKS_DISABLE_* flags, so the
 * registered tool set reflects the current environment.
 */
